...
const GOALS = [
  { minutes: 5, label: '5 min / day', desc: 'Light — great for beginners', emoji: '🌱' },
  { minutes: 15, label: '15 min / day', desc: 'Regular — steady progress', emoji: '📚' },
  { minutes: 30, label: '30 min / day', desc: 'Focused — recommended', emoji: '🎯' },
  { minutes: 60, label: '1 hour / day', desc: 'Intensive — fast results', emoji: '🚀' },
]

const REMINDER_TIMES = [
  { label: 'Morning', hour: 8, minute: 0, emoji: '🌅' },
  { label: 'Lunchtime', hour: 12, minute: 30, emoji: '☀️' },
  { label: 'Evening', hour: 19, minute: 0, emoji: '🌙' },
  { label: 'Night', hour: 21, minute: 0, emoji: '🌛' },
]
    ...
    <SafeAreaView style={[s.container, { backgroundColor: C.bg }]}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Text style={[s.subtitle, { color: C.text2 }]}>
          You're starting at <Text style={{ color: C.primary, fontFamily: Fonts.bold }}>{level}</Text>. How much time can you commit each day?
        </Text>
        ...
        <View style={s.grid}>
          {GOALS.map(g => {
            ...
            return (
              <TouchableOpacity
                key={g.minutes}
                style={[s.goalCard, { borderColor: active ? C.primary : C.border, backgroundColor: active ? C.primary + '12' : C.surface }]}
              ...
              >
                ...
                {active && <View style={[s.goalCheck, { backgroundColor: C.primary }]}><Text style={{ color: '#FFF', fontSize: 10 }}>✓</Text></View>}
              </TouchableOpacity>
            )
          })}
        </View>
        ...
        <View style={{ gap: 10 }}>
          {REMINDER_TIMES.map((rt, i) => {
            ...
            return (
              <TouchableOpacity
                key={i}
                style={[s.reminderRow, { borderColor: active ? C.primary : C.border, backgroundColor: active ? C.primary + '12' : C.surface }]}
                onPress={() => setReminderIdx(i)}
              >
                <Text style={s.reminderEmoji}>{rt.emoji}</Text>
...
const s = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 11, fontFamily: Fonts.semibold, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  goalCard: { width: '47.5%', borderRadius: 16, padding: 16, borderWidth: 1.5, gap: 4 },
  ...
  reminderRow: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14, borderWidth: 1.5 },
  ...
  footer: { paddingHorizontal: 20, paddingTop: 12, paddingBottom: 24, borderTopWidth: 1, gap: 10 },
